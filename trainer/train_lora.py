"""
train_lora.py —— MokioMind 的 LoRA 微调脚本

在项目中的角色：
  - 以预训练/全量 SFT 权重为起点，通过 `apply_lora()` 注入 LoRA（A/B 低秩增量），并冻结非 LoRA 参数。
  - 训练数据使用 `dataset.lm_dataset.SFTDataset`，即对话会被渲染成完整 input 序列，再通过稀疏 labels 只对 assistant 回复位置计算 loss。
  - 训练过程中 forward 仍走模型内置的 CE loss（labels 存在时），如启用 MoE 则可能额外返回 `aux_loss`。
  - 训练结束：调用 `save_lora()` 仅保存 LoRA 子模块的权重；同时会通过 `lm_checkpoint()` 保存断点续训需要的训练状态（optimizer/scaler 等）。

关键差异（相对 train_full_sft.py）：
  - LoRA 参数注入 + 冻结：只把名字包含 `"lora"` 的参数设为 `requires_grad=True`，optimizer 也只传入 LoRA 参数列表。
  - 梯度裁剪：clip_grad_norm_ 只作用于 `lora_params`，其他参数被冻结不会产生梯度更新。
  - 由于参数量更小，该脚本通常更省显存、更适合快速迭代不同任务 LoRA。
"""
...
def train_epoch(epoch, loader, iters, lora_params, start_step=0, wandb=None):
    ...
    数据/张量来源（来自 `SFTDataset.__getitem__`）：
      - input_ids:      [batch_size, max_seq_len]，token id 序列
      - labels:         [batch_size, max_seq_len]，稀疏标签（仅 assistant 回复 token 位置为真实 id，其余为 -100）
      - attention_mask: [batch_size, max_seq_len]，1=有效 token，0=padding

    训练要点：
      1. 只更新 LoRA 参数：
         - 外部已冻结非 LoRA 参数（requires_grad=False）
         - optimizer 也只包含 `lora_params`
         - 梯度裁剪 clip_grad_norm_ 只对 `lora_params` 生效
      2. 损失计算：
         - 调用 model(..., labels=labels, attention_mask=attention_mask) 得到 res.loss（CE loss）
         - 如模型启用 MoE，res.aux_loss 会被加入总 loss
      3. 梯度累积与混合精度：
         - loss /= args.accumulation_steps 后 backward
         - 达到累积步阈值再 unscale / clip / step / update
    """
